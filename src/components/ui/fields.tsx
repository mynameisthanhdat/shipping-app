...
export const FIELD_CLASS =
  'w-full rounded-lg border border-black/10 bg-white px-4 py-3 text-sm text-ink placeholder:text-muted-placeholder focus:border-ink/25 focus:outline-none focus:ring-2 focus:ring-brand/20';
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-placeholder transition-colors hover:text-ink"
      >
        <EyeIcon off={visible} />
      </button>
    </div>
...
export const SECTION_TITLE = 'text-[15px] font-bold uppercase tracking-wide text-ink';
...
export const Checkbox: React.FC<CheckboxProps> = ({ label, className = '', ...rest }) => (
  <label className={`flex items-start gap-2.5 text-sm text-ink ${className}`}>
    <input
      {...rest}
      type="checkbox"
...
export const Radio: React.FC<RadioProps> = ({ label, className = '', ...rest }) => (
  <label className={`flex cursor-pointer items-center gap-2 text-sm text-ink ${className}`}>
    <input {...rest} type="radio" className="h-3 w-3 shrink-0 accent-ink" />
    <span>{label}</span>
  </label>
);
...
export const Select: React.FC<SelectProps> = ({
  ...
  <div className="relative">
    ...
    <svg
      ...
      strokeLinejoin="round"
      aria-hidden="true"
      className="pointer-events-none absolute right-5 top-1/2 h-4 w-4 -translate-y-1/2 text-ink"
    >
      <path d="m6 9 6 6 6-6" />
    </svg>
  </div>
